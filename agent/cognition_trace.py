...
def build_cognition_turn_trace(metadata: Mapping[str, Any] | None) -> dict[str, Any]:
    ...
    return {
        ...
        "process_monitor": {
            "enabled": _as_bool(meta.get("process_monitor_enabled"), False),
            "claim_count": int(meta.get("process_monitor_claim_count") or 0),
            "supported_count": int(meta.get("process_monitor_supported_count") or 0),
            "evidence_gap_count": int(meta.get("process_monitor_evidence_gap_count") or 0),
            "policy_gap_count": int(meta.get("process_monitor_policy_gap_count") or 0),
            "claim_kinds": _as_str_list(meta.get("process_monitor_claim_kinds")),
        ...
        "autonomy": {
            "enabled": _as_bool(meta.get("autonomy_enabled"), False),
            "level": _as_optional_str(meta.get("autonomy_level")) or "observe",
            "competence_band": _as_optional_str(meta.get("autonomy_competence_band")) or "unknown",
            "risk_level": _as_optional_str(meta.get("autonomy_risk_level")) or "low",
            "external_action": _as_bool(meta.get("autonomy_external_action"), False),
            "approval_required": _as_bool(meta.get("autonomy_approval_required"), False),
            "approval_present": _as_bool(meta.get("autonomy_approval_present"), False),
            "evidence_required": _as_bool(meta.get("autonomy_evidence_required"), False),
            "evidence_present": _as_bool(meta.get("autonomy_evidence_present"), False),
            "policy_supported": _as_bool(meta.get("autonomy_policy_supported"), False),
            "intervention_reasons": _as_str_list(meta.get("autonomy_intervention_reasons")),
            "self_model_notes": _as_str_list(meta.get("autonomy_self_model_notes")),
        },
    }
